...
export default function Dashboard() {
  ...
  const StyledDrawer = styled(SwipeableDrawer)(() => ({
    '& .MuiDrawer-paper': {
      padding: '20px 20px',
      width: isMobile ?'' :'40vw',
      margin: isMobile ?'80% 1rem' :'25% 1rem',
      borderRadius: '10px',
      height: isMobile ? '40vh':'44vh',
      color: 'gray'
    },
  ...
  return (
    <Grid container mt={7.5} spacing={{ xs: 0, sm: 0, md: 0 }} sx={{ '&.MuiGrid-root': { width: '100%', marginLeft: '0px' } }} columns={{ xs: 3.3, sm: 12, md: 20 }}>
      ...
      <StyledDrawer anchor='left' open={showDrawer} onClose={toggleDrawer()} onOpen={toggleDrawer()}>
        ...
        <Stack direction={'row'}>
          <img src="https://cdn-icons-png.flaticon.com/512/1047/1047711.png" style={isMobile ? { 'height': '8vh' }:{ 'width': '5vw' }} alt="" />
          <Typography sx={{ margin: isMobile ?'0rem 1rem' :'1rem 1rem', padding: isMobile ?'0rem 1rem' :'0rem 3rem', fontSize:isMobile?'14px':'16px' }}>This website uses cookies to ensure you get the best experience on our website. <Link sx={{ cursor: 'pointer' }}>Learn more.</Link></Typography>
        </Stack>
        <Stack spacing={2} margin={isMobile ?'1rem 1rem 0 1rem' :'3rem 3rem 0 3rem'}>
          <Button variant="contained" sx={{fontSize:isMobile?'10px':'15px'}} onClick={() => { setShowDrawer(!showDrawer), localStorage.setItem('cookies', true) }}>Accept all cookies</Button>
          <Button variant="outlined" sx={{fontSize:isMobile?'10px':'15px'}} onClick={() => setShowDrawer(!showDrawer)}>Customize setting</Button>
        </Stack>
      </StyledDrawer>
      <ImageListItem sx={{ margin: '2rem 0rem', display: 'flex', flexDirection:'row'}}>
        <img src={banner2} style={{ margin: 'auto', width: isMobile ? '75%':'30%' }} id="hover-effect" />
      ...
      <Grid container spacing={12} padding={isMobile ? '0rem 3rem' :'0 6rem'} direction={'row'}>
